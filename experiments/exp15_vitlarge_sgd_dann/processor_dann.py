"""
DANN Processor for exp15_vitlarge_sgd_dann
Based on part_attention_vit_processor.py — adds camera adversarial loss.

Key changes from original:
1. camera_classifier + lambda_dann parameters added
2. Progressive lambda schedule (starts near 0, ramps to lambda_dann)
   - Allows classifier to learn cameras first (90%+ cam_acc)
   - Then GRL fights it down toward 40%
3. DANN branch: grad_reverse -> camera_classifier -> camera_loss
4. total_loss = reid_loss + l_ploss*ploss + lambda_effective * camera_loss
5. Logs cam_acc and lambda_effective each iteration

IMPORTANT:
- GRL handles gradient negation - camera_loss is ADDED (not subtracted)
- camera labels = camid - 1 (camid is 1-indexed, classifier expects 0-indexed)
- PC_LOSS controlled by config (True works with SGD on RTX 2080 Ti)
- Progressive lambda: 2/(1+exp(-10*p))-1 scaled by lambda_dann
"""
...
def get_lambda(epoch, max_epochs, lambda_dann):
    """
    Progressive lambda schedule from original DANN paper.
    p goes from 0->1 as training progresses.
    lambda_effective = lambda_dann * (2/(1+exp(-10*p)) - 1)

    At epoch 1/60:  lambda_effective ~ 0.034  (classifier learns freely)
    At epoch 10/60: lambda_effective ~ 0.15   (moderate pressure)
    At epoch 30/60: lambda_effective ~ 0.19   (near target)
    At epoch 60/60: lambda_effective ~ 0.20   (full pressure)

    This is why Aymen saw cam_acc start at 90%+ then decrease.
    """
    p = epoch / max_epochs
    return lambda_dann * (2.0 / (1.0 + np.exp(-10.0 * p)) - 1.0)


def part_attention_vit_do_train_with_amp_dann(
        cfg,
        model,
        train_loader,
        val_loader,
    ...
    logger.info('start training with DANN (target lambda={})'.format(lambda_dann))
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Progressive lambda — small at start, grows toward lambda_dann - I HAVE REMOVED THIS BECAUSE THIS CAN LEAD TO UNSTABLE TRAINING
        # lambda_effective = get_lambda(epoch, epochs, lambda_dann)
        ...
        for n_iter, informations in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                ...
                # layerwise_global_feat[-1] = CLS token before bottleneck
                # Shape: (B, 1024)
                # Progressive lambda: small early -> full pressure later
                # ----------------------------------------------------------
                ...
                # total_loss = reid_loss + l_ploss * ploss + lambda_effective * camera_loss THIS IS WRONG
                total_loss = reid_loss + l_ploss * ploss + camera_loss

            scaler.scale(total_loss).backward()

            # Gradient clipping for SGD
        ...
        logger.info("Epoch {} done. lambda_eff={:.4f} cam_acc={:.3f} "
                    "Time/batch:{:.3f}s Speed:{:.1f}s/s".format(
