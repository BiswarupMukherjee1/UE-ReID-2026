...
class StochasticAugmentation:
    """
    Stochastic augmentation inspired by DataCentric paper (Diaz-Benito et al.).

    Adds perspective distortion and rotation ON TOP of PAT's existing
    Color Jitter and RPT augmentations.

    NOTE: Combined system is more aggressively augmented than the paper's exact
    0.15/0.45/0.40 distribution, which was calibrated for ResNet-50 with no CJ/RPT.
    Here we add viewpoint-relevant augmentations (perspective, rotation) which are
    directly motivated by the c004 camera reversal in our task.

    Pool: perspective distortion + rotation (CJ/RPT already handled by PAT config).
    """

    def __init__(self):
        self.aug_pool = [
            TF_aug.RandomPerspective(distortion_scale=0.15, p=1.0, fill=128),
            TF_aug.RandomRotation(degrees=10, fill=128),
        ]

    def __call__(self, img):
        r = random.random()   # 'random' imported at top of file — no __import__ call
        if r < 0.15:
            return img                                # no augmentation
        elif r < 0.60:                                # p=0.45: one aug
            return random.choice(self.aug_pool)(img)
        else:                                         # p=0.40: two augs
            aug1, aug2 = random.sample(self.aug_pool, 2)
            return aug2(aug1(img))
...
if __name__ == '__main__':
    ...
    logger.info("Experiment: exp21_224x224")
