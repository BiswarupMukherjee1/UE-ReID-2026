# train_continue_exp13.py
# Same as train.py but loads FULL checkpoint (including classifier + bottleneck)
# using load_param_finetune instead of load_param.
# This correctly continues training from exp13 epoch 40 checkpoint.
# load_param (used by PRETRAIN_CHOICE:self) skips classifier and bottleneck
# which would cause catastrophic forgetting — this file fixes that.
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="ReID Training - Continue from checkpoint")
    parser.add_argument(
    ...
    parser.add_argument("--resume", default="", type=str,
                        help="path to checkpoint to resume from (loads ALL weights)")
    ...
    # ── RESUME: load ALL weights including classifier + bottleneck ──────────
    resume_path = args.resume
    if not resume_path:
        resume_path = cfg.MODEL.PRETRAIN_PATH if cfg.MODEL.PRETRAIN_CHOICE == 'self' else ''
    if resume_path and os.path.isfile(resume_path):
        logger.info("Resuming from checkpoint: {}".format(resume_path))
        model.load_param_finetune(resume_path)
        logger.info("Loaded ALL weights (classifier + bottleneck included)")
    else:
