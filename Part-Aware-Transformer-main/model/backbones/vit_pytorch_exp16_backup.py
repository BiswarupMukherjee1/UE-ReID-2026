...
class part_Attention_ViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.view_embedding = nn.Parameter(torch.zeros(2, embed_dim))
        self.lambda_view    = 5.0 # this might be wrong
    ...
    def forward_features(self, x, view_ids=None):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        if view_ids is not None:
            ...
            x = x.clone()
            x[:, 0] = x[:, 0] + self.lambda_view * view_embed
        layerwise_tokens = []
    ...
    def forward(self, x, view_ids=None):
        x = self.forward_features(x, view_ids=view_ids)
        return x
